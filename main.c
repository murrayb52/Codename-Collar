...
#define		level2	50
	...
	// Resets/Presets
// this is the reset state for GPIOA
...
void init_ports (void)	{
	...
	GPIOA->MODER &= ~GPIO_MODER_MODER0; 			// set PA0 to input 				chan_interrput
...
/* NOTE: Setting PB1 to output automatically outputs the reset value [zero] which will send a 'transmit' signal. However, since this is one of the first steps
		...
		period of time (a period whereby the action is practically negligible).
		*/

	GPIOA->ODR |= 0b111010;							// reset outputs to remote buttons (transmit, level_up, mode_selr & chselr pull-up pins)
	delay(3);
	GPIOB->ODR |= (0b1<<led_wait);					// turn on 'initialise LED' (and reset all other outputs)
...
/*	if (mode_preset == 1)	{
		toggle(3);									// toggle mode_selr button 		(enter light mode)
		}
*/
...
*/
	channel_count += 1;
	channel_count = (channel_count%2				// update 'channel_count' value
	GPIOA->ODR &= ~(0b11<<chan);					// reset 'chan' indicator LED
	GPIOA->ODR |= (channel_count<<chan);			// set 'chan' indicator LED
}
...
void transmit (int channel_select)	{
	...
	//GPIOA->ODR &= ~(0b11<<chan);								// clear channel transmit indicators
	//GPIOA->ODR |= ((0b01 + channel_count)<<chan);				// PB6: channel 1; PB7: channel 2
	delay(2);
	while ((GPIOA->IDR & (0b1<<(channel_select - 1))) == 0)	{	// while activate button held
		GPIOA->ODR &= ~(0b1<<transmit_pin);						// transmit signal [CONTINUOUSLY]
		GPIOA->ODR |= (0b1<<led_transmit);						// activate transmission LED
	}
	...
	GPIOA->ODR &= ~(0b11<<chan);								// clear channel transmit indicators
	GPIOA->ODR &= ~(0b1<<led_transmit);							// deactivate transmission LED
}
