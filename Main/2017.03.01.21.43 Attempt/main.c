...
/* NOTE: Setting PB1 to output automatically outputs the reset value [zero] which will send a 'transmit' signal. However, since this is one of the first steps
	...
	delay(2);
	GPIOB->ODR |= 0b11110;							// reset level_up, mode_selr, chselr & transmit pull-up pins
	delay(2);
...
// Channel selector button: (set default start = channel 1; current = channel 1)		[cycle = channel 1, channel 2]
	//toggle(2);									// toggle chselr button 		(enter channel 1 [start channel])
...
void preset (void)	{
	...
	*/
	...
	level(5);										// set channel 1 to 50 shock
	...
	level(7);										// set channel 2 to 50 shock
...
void update_mode(void)	{
	int moder;
	mode_count += 1;
	mode_count = (channel_count%4);
	moder = (mode_count%2);
	GPIOB->ODR &= ~(0b1<<mode);
	delay(3);
	GPIOB->ODR |= (moder<<mode);
	delay(3);
	}
