...
//	if ((GPIOB_ODR & 'mode press' signal) != 0): this is due to user input and should be accounted for																	//  
		delay(1);
		if (((GPIOB->ODR) & (0b1<<chselr)) != 0) 	{				// check whether interrupt due to micro-controller or user
...
//	if ((GPIOB_ODR & 'mode press' signal) != 0): this is due to user input and should be accounted for																	//  
		delay(1);
		if (((GPIOB->ODR) & (0b1<<mode_selr)) != 0) 	{				// check whether interrupt due to micro-controller or user	
			update_mode();												// update mode_count value
		}																// else, ignore because button 'pressed' by micro - logic hard-coded for specific scenario
